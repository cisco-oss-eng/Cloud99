...
def nova_endpoint_check(nova_instance):
    ...
    if  status == 200:
        for service in service_list:
            cpulse_log.debug("Binary=%s Host=%s Zone=%s Status=%s State=%s", service.binary, service.host,
...
def keystone_endpoint_check(keystone_instance):
    ...
    if status == 200:
        for service in service_list:
            cpulse_log.debug("Service=%s enabled=%s", service.name, service.enabled)
...
def neutron_endpoint_check(neutron_instance):
    ...
    if status == 200:
        for agent in agent_list:
            cpulse_log.debug("Agent=%s Host=%s Alive=%s admin_state_up=%s",  agent['binary'], agent['host'],
...
def cinder_endpoint_check(cinder_instance):
    ...
    if status == 200:
        #logging("cinder list:%s", cinder_list)
